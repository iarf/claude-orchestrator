  ...
  return [
    {
      name: "dispatch_task",
      description:
        "Dispatch a task to a Claude Code agent. Returns immediately with a task ID. " +
    ...
    },
    {
      name: "get_status",
      description:
        "Get the current status of a dispatched task. Returns status, timing info, and turn count.",
      inputSchema: {
        type: "object" as const,
        properties: {
    ...
    },
    {
      name: "get_result",
      description:
        "Get the output of a completed (or failed) task. Returns the agent's stdout. " +
    ...
    },
    {
      name: "list_tasks",
      description:
        "List all tasks with optional status filter. Returns summary info for each task.",
      inputSchema: {
        type: "object" as const,
        properties: {
          status: {
    ...
    },
    {
      name: "get_policy",
      description:
        "Returns the current permission policy: allowed paths, blocked tools, and blocked command patterns. " +
    ...
    switch (name) {
      ...
      case "cancel_task": {
        ...
        if (task.status === "queued") {
          ...
          return {
            content: [{ type: "text" as const, text: `Task ${taskId} cancelled (was queued).` }],
          };
        }
      ...
      case "list_tasks": {
        ...
        const tasks =
          status === "all"
            ? taskManager.listAll()
            : taskManager.listByStatus(status as TaskStatus);
